import { WithdrawalType } from "@/types";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Eye } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import TableLoading from "@/components/shared/TableLoading";
import { formatNumber } from "@/lib/utils";

interface WithdrawalTableProps {
  withdrawals: WithdrawalType[];
  isLoading: boolean;
  onView: (withdrawal: WithdrawalType) => void;
  currentPage?: number;
  perPage?: number;
}

const statusVariants: Record<string, string> = {
  pending: "bg-yellow-500/10 text-yellow-500 border-yellow-500/20",
  approved: "bg-green-500/10 text-green-500 border-green-500/20",
  rejected: "bg-red-500/10 text-red-500 border-red-500/20",
};

const WithdrawalTable = ({
  withdrawals,
  isLoading,
  onView,
  currentPage = 1,
  perPage = 10,
}: WithdrawalTableProps) => {
  return (
    <Table>
      <TableHeader>
        <TableRow>
          <TableHead className="w-[60px]">Sr</TableHead>
          <TableHead>User</TableHead>
          <TableHead>Amount (Coins)</TableHead>
          <TableHead>Account Name</TableHead>
          <TableHead>Account Number</TableHead>
          <TableHead>Payment Type</TableHead>
          <TableHead>Status</TableHead>
          <TableHead>Date</TableHead>
          <TableHead className="text-right">Actions</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {isLoading ? (
          <TableRow>
            <TableCell colSpan={9} className="h-64">
              <TableLoading />
            </TableCell>
          </TableRow>
        ) : withdrawals.length === 0 ? (
          <TableRow>
            <TableCell colSpan={9} className="h-64 text-center">
              <p className="text-muted-foreground">No withdrawals found.</p>
            </TableCell>
          </TableRow>
        ) : (
          withdrawals.map((withdrawal, index) => (
            <TableRow key={withdrawal.id}>
              <TableCell className="text-sm text-muted-foreground">
                {(currentPage - 1) * perPage + index + 1}
              </TableCell>
              <TableCell>
                <div className="flex flex-col">
                  <span className="font-medium text-sm">{withdrawal.user.name}</span>
                  <span className="text-xs text-muted-foreground">
                    {withdrawal.user.phone || withdrawal.user.email}
                  </span>
                </div>
              </TableCell>
              <TableCell className="font-semibold text-red-500">
                {formatNumber(withdrawal.coin_amount)}
              </TableCell>
              <TableCell className="text-sm font-medium">
                {withdrawal.account_name}
              </TableCell>
              <TableCell className="text-sm font-mono">
                {withdrawal.account_number}
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-2">
                  <Avatar className="size-6">
                    <AvatarImage src={withdrawal.payment_type.logo || ""} />
                    <AvatarFallback className="text-[10px]">
                      {withdrawal.payment_type.name.substring(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <span className="text-sm">{withdrawal.payment_type.name}</span>
                </div>
              </TableCell>
              <TableCell>
                <Badge
                  variant="outline"
                  className={statusVariants[withdrawal.status]}
                >
                  {withdrawal.status.toUpperCase()}
                </Badge>
              </TableCell>
              <TableCell className="text-sm text-muted-foreground">
                {withdrawal.created_at}
              </TableCell>
              <TableCell className="text-right">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => onView(withdrawal)}
                  className="size-8"
                >
                  <Eye className="size-4" />
                </Button>
              </TableCell>
            </TableRow>
          ))
        )}
      </TableBody>
    </Table>
  );
};

export default WithdrawalTable;
